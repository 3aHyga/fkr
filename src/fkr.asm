...
0x3fe6 = 0x7000; установка 3-х циклов ожидания при чтении из регистра данных от камеры
DM(I4,M5) = offset bufferdata; установка адреса входного регистра BIAD в bufferdata
DM(I4,M5) = 0x2000; установка адреса входного регистра BEAD в 0x2000
DM(I4,M5) = 3; установка режима чтения из ПД в режиме МЗБ
DM(I4,M5) = n; загрузка в регистр счетчика BWCOUNT количества требуемых слов и запуск загрузки
...
start1: I4 = 0x3FE1;		/* 0x0008: IRQL1*/
	...
	IDLE;			/* 0x0010: SPORT0 передача, ожидание прервывания окончания закрузки программы из БП */
...
int0_loop_exit: DM(I3,M1) = AX1;
	...
	DM(I4,M5) = 6;		/* запуск копирования из ПД в БП */
